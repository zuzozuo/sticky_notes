class WhiteBoardController {
	...
	run() { //inicjalizacja karteczki, przywrocenie ich z bazy + wysylanie danych , podpiecie eventow, dodanie karteczek do tablic
		let that = this;
	...
	cancelEdition() { //anuluj edycje
		document.getElementById("tinyMCE").style.display = "none";
	}

	getTextEditor(event) { //ustawianie edytora tektu dla karteczki
	...
	changeText() { //zmiana tekstu, aktualizacja, chowaie sie edytora po zmianie
	...
	addNewSticky() { //dodawanie nowej karteczki
	...
	removeFromDB(uid) { //usuwanie karteczki z bazy DELETE
		let that = this;
		let obj = {
			uid: uid
		}
	...
	removeSticky(event) { //ussuwanie karteczki z listy i przetasowanie zIndexow
		for (let i = 0; i < this.listOfNotes.length; i++) {
	...
	changeActiveSticky(event) { //zmiana aktywnej karteczki i ustawienie na najwyzzy indeks
